...
export const CheckAuth = ({token,user,isAuthenticated,children,initialized}) => {
  // console.log(token,user["role"],isAuthenticated)
  ...
  if(isAuthenticated && (location.pathname.includes("/login") || location.pathname.includes("/register"))){
    // as "user" is stored in localStorage as string, so we need to extract user["role"] to get user.role
